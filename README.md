...
SREHARE NAIDU A/L VASU — Diagnostic Analysis & Feature Development
Designed the morphological analysis module, incorporating circularity measurements to classify objects based on shape.
Developed the diagnostic labeling system, including the visual color-coding scheme (Green = circle, Red = capsule) guided by regularity benchmarks.
Enhanced the visual output design, such as the 4-panel comparison view and automated text-labeling for count totals and regularity metrics.
Joint Contribution
Both team members closely collaborated during the Watershed tuning and debugging stage, working together to optimize the separation parameters so that touching pills could be accurately identified and counted.
